...
func _process(_delta: float) -> void:
	if interactor:
		...
		var t = clamp((fade_start - distance) / (fade_start - fade_end), 0.2, 1.0)
		var col = interact_hint_bubble.modulate
		col.a = t
		interact_hint_bubble.modulate = col
...
func show_hint_bubble(interactor_: InteractionComponent, hint_visible_radius: float) -> void:
	...
	interact_hint_bubble.visible = true
...
func show_hint_ring() -> void:
	interact_hint_ring.visible = true
